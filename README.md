...
微备份（WxBackup）是一款面向NAS的**微信备份/恢复**工具(付费)，支持**鸿蒙/苹果/安卓手机**备份微信数据至NAS，支持把微信数据从NAS恢复到**鸿蒙/苹果/安卓手机**。实现长期保存与随时查看数据。支持全量/增量备份、全量/部分恢复。
...
* Docker版：**即将上线（原计划是春节前上线，因不可抗力因素，继续延后~😭😭😭）**
...
**5、微备份v1.2.0.0（v1.2.0）开始 已支持把微信数据从NAS恢复到鸿蒙/苹果/安卓手机。** 

**6、如遇到“无法登录/备份、更换/重装设备、无昵称、无头像、恢复备份、会员权益”等【常见问题】往下翻即可找到。** 

**以下是微备份Web端的部分界面截图：**

![1780539756032](assets/1780539756032.png)
...
![1780539859997](assets/1780539859997.png)
...
![1780539394015](assets/1780539394015.png)
...
![1780539510594](assets/1780539510594.png)
...
2. **支持Nas**：无需越狱、Root即可将聊天记录备份至Nas设备，也可以从Nas恢复至手机，目前已支持飞牛OS，后续会支持更多的主流平台。
...
6. **支持恢复聊天记录**：微备份v1.2.0版本开始新增了恢复功能，亦可使用Windows端的微信进行恢复已备份的聊天记录（文档最后有恢复教程）。
...
2、在 **[点击这里进入Github下载👉️微备份（WxBackup），选择“WxBackup_X.X.X.X.fpk”名字的安装包](https://github.com/weibeifen/wxbackup/releases/)**  下载保存微备份的安装包
...
![1780538170273](assets/1780538170273.png)
...
![1780540684640](assets/1780540684640.png)
...
![1780541610522](assets/1780541610522.png)
...
![1780541682741](assets/1780541682741.png)
...
![1780541747874](assets/1780541747874.png)
...
![1780541796375](assets/1780541796375.png)

（整理数据中）

![1780542379970](assets/1780542379970.png)

（更新数据中）

![1780542437248](assets/1780542437248.png)



###### 备份完成

![1780542653437](assets/1780542653437.png)
...
1、必须认真查看上述的 **”使用注意事项（非常重要）”** 的内容后才能登录，否则可能出现恢复失败的情况。

2、**恢复时间可能会很长（聊天记录越多，恢复时间越长），需要耐心等待，** 最好选一个空闲的时间段进行恢复。

3、**恢复过程中，依然可以正常使用手机端微信。**

4、**需要保持手机端微信在前台（即保持微信处于打开状态），不能切换到后台，否则会恢复失败。**
...
![1780545098496](assets/1780545098496.png)
...
![1780545214224](assets/1780545214224.png)



###### 正在恢复（恢复速度比较慢，请耐心等待）

恢复时长是由微信聊天记录大小决定，聊天记录越大，恢复越慢。最好是选择一个空闲的时间段再进行恢复。

以下是恢复时的截图：

（正在恢复中）

![1780545300126](assets/1780545300126.png)



###### 恢复完成

![1780545622261](assets/1780545622261.png)
...
![1780544314062](assets/1780544314062.png)



#### 关于会员权益的说明
* 一个微信号只能购买一个会员，多个微信号需购买多次会员，一个微信号对应一个会员，购买后即为年费会员；
* 购买会员后，在会员有效期内该微信号支持无限次备份（不管是完整备份还是增量备份），而非会员只能试用1次备份；
* 不管是否会员，备份后，均能使用当前已有的功能；
* 会员与微信号一一绑定，换Nas设备（目前仅支持飞牛OS，后续会支持其他平台）并不影响使用，只需重新安装微备份，登录即可备份；
* 目前的会员权益是有效期内无限次备份，未来是否会增加权益，在不断新增功能中；
* 修改微信号，并不会影响会员，依然是绑定关系；
* 年费会员过期后，无法登录、备份、恢复，但依然可以查看已备份的聊天记录内容，非鸿蒙微信如需紧急恢复，可以在【常见问题】最后一部分找到如何恢复。

![1783493673340](assets/1783493673340.png)
...
![1778056888275](assets/1778056888275.png)
...
#### 对于鸿蒙微信，恢复聊天记录只能是使用【微备份自带的恢复功能】，如下图所示
![1781596194966](assets/1781596194966.png)

#### 对于非鸿蒙微信，可使用【微备份自带的恢复功能】进行恢复，也可以使用微信官方PC端进行恢复。非鸿蒙微信的恢复教程就省略了，在上述介绍功能时，已介绍如何使用自带的恢复功能。以下是使用微信官方PC端进行恢复的教程。
