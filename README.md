...
**1、鸿蒙微信备份失败，首先先检查一下8011端口是否被占用，如果被占用，先停用占用的应用，使用完微备份后，再恢复原使用8011端口的应用。（更多详情请看下面的【常见问题】）** 
...
**4、微备份v1.1.0.0（v1.1.0）开始 已支持鸿蒙版微信（保证8011端口不被占用）。** 
...
出现这种情况，可能是因为微备份所使用的8011端口被占用了，这个8011端口是微信官方所使用的端口，所以我们没法更改。您只能是先停用(应用中心-XXX应用-停用)其他占用8011端口的应用，等备份完后，再启用(应用中心-XXX应用-启用)原使用8011端口的应用。
